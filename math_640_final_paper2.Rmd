...
```{r}
# calculate a distribution of log liklihoods under the model
main_p <- apply(main_posterior$theta, 1, function(x){
  dmultinom(y, prob = x, log = TRUE)
})

control_p <- apply(control_posterior$theta, 1, function(x){
  dmultinom(y, prob = x, log = TRUE)
})
