...
```
As discussed above, to sample from these conditional distributions, we had to create Gibb's samplers using metropolis-hastings components. For both the control and the man model, we used an Inverse-Gaussian to sample from the condtional posterior on $\alpha$, and sampled directly from the conditional posteriors of the other paramters. While the acceptance rates for $\alpha$ are not ideal, they did provid....Short description of acceptance rates here....must finalize proposal for control...... Figure 2 plots the density of the acceptance rates, showing the majority of the mass in an indeal range. 
...
```
\begin{center} Figure 2: Plot of acceptance rates of $\alpha$. \end{center}
...
kable(data.frame(g),digits = 2, caption = "Geweke statistic of log likelihoods")
```
