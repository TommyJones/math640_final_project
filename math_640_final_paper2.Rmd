...
Of these conditional posteriors, only $\vec{\alpha}|\vec{\theta},\beta,\vec{y}$ is an unknown distribution. $\vec{\theta}|\vec{\alpha},\beta,\vec{y}$ is a $Dir(\vec{y}+\vec{\alpha})$, and $\beta|\vec{\theta},\vec{\alpha},\vec{y}$ is a Gamma$\bigg(k, \sum_k\log(\alpha_k)-k\log(\gamma)\bigg)$. The other two conditionals will be sampled using a Metropolis-Hastings algorithm with another exponential proposal.
...
```{r}
# calculate a distribution of bayes factors
# note: main model is in the numerator
...
d <- nrow(main_posterior$theta)

for (j in 1:B) {
  bf[j] <- dmultinom(y, prob = main_posterior$theta[sample(seq_len(d),1),], log = T) - 
                 dmultinom(y, prob = control_posterior$theta[sample(seq_len(d),1),], log = T)
}
