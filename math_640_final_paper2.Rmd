...
For the comparison of the models, we calculated the deviance information criterion for each estimate of $\alpha$, creating a distribution of DICs. This distribution is shown in figure 2. We also provide the plot of the densities of the log-likelihood of each estimate of $\alpha$ (Figure 2). 

# Results 

Figures 2 and 3 provide fairly conclusive evidence that the main model does is not better than the control. Figure XX DIC plot shows that the control model has a much lower DIC, and the credible intervals for these estimates do not overlap. This suggests that the control is a better model. The log-likelihood plot also shows that the control model is more likely, although with some overlap of distribution.  
...
```{r model_plots, include = T, out.width='.49\\linewidth', fig.width=5, fig.height=4,fig.show='hold',fig.align='center', fig.cap='Histogram of Word counts (left) and  Log-log plot of word counts, ordered by rank (right).'}
