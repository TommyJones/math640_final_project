...
\begin{center}Figure 1:Histogram of Word counts (left) and  Log-log plot of word counts, ordered by rank (right). \end{center}
### Samplers
...
kable_styling(kable(alpha_acc_table, format = "latex", booktabs = T, caption = "Table 1: Acceptance Rates"),position = "float_right")

```
As discussed above, to sample from these conditional distributions, we had to create Gibb's samplers using metropolis-hastings components. For both the control and the man model, we used an Inverse-Gaussian to sample from the condtional posterior on $\alpha$, and sampled directly from the conditional posteriors of the other paramters. This is whereThis is whereThis is whereThis is whereThis is whereThis is whereThis is whereThis is whereThis is whereThis is whereThis is whereThis is whereThis is whereThis is whereThis is whereThis is whereThis is whereThis is whereThis is whereThis is whereThis is whereThis is whereThis is whereThis is whereThis is whereThis is whereThis is whereThis is whereThis is whereThis is whereThis is whereThis is whereThis is whereThis is whereThis is whereThis is whereThis is whereThis is whereThis is whereThis is whereThis is whereThis is whereThis is whereThis is whereThis is whereThis is whereThis is whereThis is whereThis is whereThis is whereThis is whereThis is whereThis is whereThis is whereThis is whereThis is whereThis is whereThis is whereThis is where


```{r accept_plot, fig.align = 'center', fig.width = 6.5, fig.height = 3}
...
legend("topleft", legend = c("Main", "Control"), lwd = 4, lty = c(1,3),
       col = c("red", "blue"))

```
Figure 2: Plot of acceptance rates of $\alpha$.
